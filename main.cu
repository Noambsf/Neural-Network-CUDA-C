...
int main(int argc, char *argv[])
{
    ...
    image* train_img = read_images("data/train-images.idx3-ubyte", &datasize);
    byte* train_label = read_labels("data/train-labels.idx1-ubyte", &datasize);
    image* test_img = read_images("data/t10k-images.idx3-ubyte", &ntest);
    byte* test_label = read_labels("data/t10k-labels.idx1-ubyte", &ntest);
    ...
    printf("starting accuracy %lf\n", accuracy(test_img, test_label, ntest, minibatch_size, nn)); fflush(stdout);
    ...
    for (int epoch = 0; epoch < 10; epoch ++)
    {
        ...
        { 
            ...
            backward_gpu(nn, out, dsigmoid);
            if (i % 20000 == 0)
            {
                printf("%i images sur %i ont été traitées (Epoch %i) \n", i, datasize, epoch);
            }
        }   
        end = clock();  
        time_epoch = (double)(end - start)/CLOCKS_PER_SEC;
        printf("Temps d'entraînement pour l'epoch %i : %f\n", epoch, time_epoch);
